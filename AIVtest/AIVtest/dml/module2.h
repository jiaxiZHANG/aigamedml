...
struct Box
{
	float left, top, right, bottom, confidence, x, y, w, h;
	int class_label;

	Box() = default;

	Box(float left, float top, float right, float bottom, float confidence,  float x, float y, float w, float h, int class_label)
		:left(left), top(top), right(right), bottom(bottom), confidence(confidence), x(x), y(y), w(w), h(h), class_label(class_label) {}
};
...
vector<Box> houchuli(float* feat_blob, vector<Box>& objects, int out1 ,int out2, float confidence, float  threshold)
{
